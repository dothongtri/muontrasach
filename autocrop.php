...
class AutoCrop
{
	 ...
	 * @return 	boolean
	 */
	private function cropImage($source)
	{
		...
		if(substr($fileType, 0, 5) == "image")
		{
			...
			if($this->originalImage)
			{
				...
				$rgb = imagecolorat($this->destinationImage, 0, 0);
				$colors = imagecolorsforindex($this->destinationImage, $rgb);
				$this->edge = $this->imageGetEdges($this->destinationImage, $colors['red'], $colors['green'], $colors['blue']);
				...
				return $this->outputImage($source);
			}
			else return false;
		}
		else return false;
	 ...
	 * @return boolean
	 */
	private function outputImage($source)
	{
		if(count($this->edge) == 4)
		{
			...
			{	// inline display
				...
				{
					case "image/jpeg":
						imagejpeg($this->destinationImage,$source);
						break;
						
					case "image/png":
						imagepng($this->destinationImage,$source);
						break;
						
					case "image/gif":
						imagegif($this->destinationImage,$source);
						break;	
				}
			}
			return true;
