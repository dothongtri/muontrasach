...
class compareImages
{
    ...
    {
		//hàm createImage($a) Trả về một hình ảnh hoặc là false nếu nó không phải jpg hoặc png
		$i1 = $this -> createImage($a);
		$i2 = $this -> createImage($b);
		//so sánh nếu 2 hình khác nhau sẽ return false
		if(!$i1 || !$i2){return false;}
		//hàm resizeImage($i1,$a) thay đổi kích thước hình ảnh thành hình vuông 8x8
		$i1 = $this -> resizeImage($i1,$a);
		$i2 = $this -> resizeImage($i2,$b);
		//Hàm imagefilter($i1, IMG_FILTER_GRAYSCALE) Chuyển đổi hình ảnh thành thang độ xám
